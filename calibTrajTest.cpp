...
void micLocalFunc(double *beta, double *hatacc, int Mparam, int ndata, double *data){
  ...
  int choice_model=(int)(data[4*ndata]);
...
//#############################################################################
// estimated endogeneous variable = array of gap hats (calType=1) or ln(hats) (calType=2)
//#############################################################################
...
void micGlobalFunc(double *beta, double *hats_or_lns, int Mparam, int ndata, double *data){
  ...
  sSim[0]=sdata[0];
  vSim[0]=vdata[0];
  double accmax=100; //!! for tests for new targets (accmax=100)
  Statistics stat;
  ...
  for (int i=1; i<nSim; i++){
    ...
    double acc=
     (choice_model==0)    ?   accIDM(sSim[i-1],vSim[i-1],vSim[i-1]-vlintpOld,beta)
      : (choice_model==1) ? accIDM_v0fixed(sSim[i-1],vSim[i-1],vSim[i-1]-vlintpOld,v0,beta)
...
int main(int argc, char* argv[]){
  ...
  if(true){
    ...
    if(choice_model==4){betamin[3]=4; betamax[3]=20;}
    if(choice_model==5){betamin[3]=2; betamax[3]=12;}
